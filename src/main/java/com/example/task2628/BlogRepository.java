...
public interface BlogRepository extends JpaRepository<Blog, Long> {
    @Query("SELECT b FROM Blog b WHERE CONCAT(b.name, ' ', b.date, ' ', b.blogText) LIKE %?1%")
    List<Blog> search_all(String keyword);
    @Query("SELECT b FROM Blog b WHERE CONCAT(b.name, ' ', b.date) LIKE %?1%")
    List<Blog> search_name_date(String keyword);
    @Query("SELECT b FROM Blog b WHERE CONCAT(b.name, ' ', b.blogText) LIKE %?1%")
